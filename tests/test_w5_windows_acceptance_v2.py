...
PACK = json.loads((ROOT / "tests/evals/windows_acceptance_v2/manifest.json").read_text(encoding="utf-8"))
REFERENCES = json.loads((ROOT / "tests/evals/windows_acceptance_v2/references.json").read_text(encoding="utf-8"))["answers"]
...
def page(root, value):
    ...
    second.write_text("<p>delta</p>", encoding="utf-8")
    target = root / "fixture.html"
    target.write_text(
        f"<p>{value}</p>"
        "<form onsubmit=\"document.body.dataset.saved=document.querySelector('input').value;return false\">"
        "<input><button>Save</button></form>"
        f"<a target='_blank' href='{second.as_uri()}'>Tab</a>"
        "<a download='x.txt' href='data:text/plain,payload-84'>Download</a>",
        encoding="utf-8"
    )
    return target
...
def browser(root, item):
    ...
    try:
        ...
        if kind == "form":
            ...
            obs = backend.act(session, obs["observation_id"], inp, "fill", "committed")
            button = next(k for k, v in obs["elements"].items() if v["text"] == "Save")
            backend.act(session, obs["observation_id"], button, "click")
            return backend.page(backend.sessions[session]).locator("body").get_attribute("data-saved") == "committed"
        if kind == "tabs":
        ...
        if kind == "download":
            ...
            return Path(result["path"]).read_text() == "payload-84"
        if kind == "cancel":
...
def local(root, item):
    ...
    if kind == "json":
        ...
        path.write_text('{"value":84}')
        return validate_json(path, expected={"value": 84}).passed
    if kind == "csv":
        path = root / "answer.csv"
        path.write_text("name,value\ngamma,84\n")
        return validate_csv(path, expected_rows=[{"name": "gamma", "value": "84"}]).passed
    if kind == "report":
        path = root / "report.md"
        path.write_text("Result is certified and verified.")
        return validate_report(path, required_phrases=["certified"], minimum_words=4).passed
    if kind == "process":
        broker = ToolBroker(root, {"run_process"}, constrained=False)
        result = broker.dispatch(ToolCall("p", "run_process", {"argv": [sys.executable, "-c", "print('terminal-success')"], "cwd": "."}, str(root)))
        return result.ok and "terminal-success" in result.text
    if kind == "unicode":
        path = root / "ü space.txt"
        path.write_text(item["input"]["value"], encoding="utf-8")
        return path.read_bytes() == item["input"]["value"].encode("utf-8")
    path = root / "owned.txt"
    path.write_text("protected-content")
    ...
    return not result.ok and path.read_text() == "protected-content"
...
def long(root, item):
    ...
    canary = root / "orphan_15s.txt"
    script = f"import time, pathlib; time.sleep(15); pathlib.Path(r'{canary}').write_text('bad')"
...
def test_sealed_windows_acceptance_v2_pack(tmp_path, item, repeat):
    ...
    if category == "research":
        ...
        if output.endswith(".json"):
            ...
            valid = validate_csv(tmp_path / output, expected_rows=[{"status": "active"}]).passed
        else:
            (tmp_path / output).write_text("Audit report is certified and verified.")
            valid = validate_report(tmp_path / output, required_phrases=["certified"]).passed
...
def test_v2_pack_is_sealed_complete_and_predeclared():
    tasks = PACK["tasks"]
