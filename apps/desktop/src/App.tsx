...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-panel">
      ...
      {screen === "chat" && <ChatScreen messages={messages} draft={draft} setDraft={setDraft} onSend={() => void send()} onStarter={(value) => void send(value)} streaming={streaming} activity={activity} canChat={connection.runtime_mode === "local" || connection.has_cli_token || connection.model_profile.startsWith("local:")} localModel={connection.model_profile.startsWith("local:")} connection={connection} onStartExecutor={() => void runAction(desktop.start, "Desktop executor started. Approved local work can now be claimed.")} onSignIn={() => setScreen("settings")} onOpenWeb={() => void desktop.openWeb()} onClearActivity={() => setActivity([])} transcriptEndRef={transcriptEndRef} />}
