...
function App() {
  ...
  async function send(message = draft) {
    ...
    if (!connection.has_cli_token) {
      setScreen("settings");
      setNotice("Sign in to Smara before starting a chat.");
      return;
    }
    setDraft("");
    ...
    try {
      ...
      setMessages((items) => items.map((item) => item.id === answerId ? { ...item, pending: false, failed: true, text: message } : item));
      assistantId.current = null;
    }
  }
  ...
  return <div className="app-shell">
    ...
    <main className="main-panel">
      ...
      {screen === "chat" && <ChatScreen messages={messages} draft={draft} setDraft={setDraft} onSend={() => void send()} onStarter={(value) => void send(value)} streaming={streaming} activity={activity} hasToken={connection.has_cli_token} onSignIn={() => setScreen("settings")} onOpenWeb={() => void desktop.openWeb()} />}
...
function SettingsScreen({ connection, onSaved, onPaired, onSignIn }: { connection: ConnectionState; onSaved: (next: ConnectionState) => void; onPaired: (next: ConnectionState) => void; onSignIn: (apiUrl: string, webUrl: string) => void }) {
  ...
  async function pair() {
    ...
    setPairing(true);
    setFormError(null);
    try { onPaired(await desktop.pair({ api_url: apiUrl.trim(), code: code.trim(), allowed_roots: splitLines(roots), terminal_allowlist: splitLines(terminal), browser_domains: splitLines(domains) })); setCode(""); } catch (error) { setFormError(errorMessage(error, "Pairing failed")); } finally { setPairing(false); }
  }
...
type SettingsPanelProps = {
  ...
  credentials: LocalCredentialSummary[]; credentialProvider: string; credentialName: string; credentialSecret: string; credentialBusy: boolean; formError: string | null;
  ...
  save: () => Promise<ConnectionState | null>; pair: () => Promise<void>; chooseCredentialProvider: (provider: string) => void; saveCredential: () => Promise<void>; removeCredential: (name: string) => Promise<void>; onSignIn: (apiUrl: string, webUrl: string) => void;
};

function SettingsPanel({ connection, apiUrl, webUrl, workspace, modelProfile, roots, terminal, domains, code, pairing, credentials, credentialProvider, credentialName, credentialSecret, credentialBusy, formError, setApiUrl, setWebUrl, setWorkspace, setModelProfile, setRoots, setTerminal, setDomains, setCode, setFormError, setCredentialName, setCredentialSecret, save, pair, chooseCredentialProvider, saveCredential, removeCredential, onSignIn }: SettingsPanelProps) {
  const selectedProfile = modelProfiles.find((profile) => profile.value === modelProfile) || modelProfiles[0];
  ...
  return <section className="content-page settings-page">
    ...
    <div className="settings-grid">
      ...
      <div className="settings-card"><div className="card-heading"><span className="card-icon">⌁</span><div><h3>Pair this desktop</h3><p>Link this PC to your Smara account with a one-time code.</p></div></div><div className="pair-status"><span className={`status-dot ${connection.paired ? "dot-green" : "dot-amber"}`} /><strong>{connection.paired ? "Paired and scoped" : "Not paired"}</strong>{connection.executor_id && <span>{connection.executor_id}</span>}</div><div className="pair-row"><input value={code} onChange={(event) => { setFormError(null); setCode(event.target.value.toUpperCase()); }} placeholder="8-character code" maxLength={8} spellCheck={false} /><Button kind="primary" onClick={() => void pair()} disabled={!isNativeDesktop || pairing || code.trim().length !== 8}>{pairing ? "Pairing…" : "Pair device"}</Button></div><p className="small-help">In Smara Web open Settings → Desktop → Pair device. The code is short-lived and single-use.</p></div>
      <div className="settings-card full-card model-card"><div className="card-heading"><span className="card-icon">✦</span><div><h3>Hosted model</h3><p>Pick a profile; provider keys stay on the hosted service and are never copied to this PC.</p></div><span className={`model-selected model-${selectedProfile.tone}`}>{selectedProfile.label}</span></div><div className="provider-grid" role="radiogroup" aria-label="Hosted model profile">{modelProfiles.map((profile) => <button type="button" className={`provider-option ${modelProfile === profile.value ? "provider-selected" : ""}`} key={profile.value} onClick={() => setModelProfile(profile.value)} aria-pressed={modelProfile === profile.value}><span className={`provider-mark provider-${profile.tone}`}>{profile.label.slice(0, 1)}</span><span className="provider-copy"><strong>{profile.label}</strong><small>{profile.provider}</small><span>{profile.description}</span></span><span className="provider-check">{modelProfile === profile.value ? "✓" : ""}</span></button>)}</div><div className="small-help model-help"><span>Selected:</span> {selectedProfile.label} · {selectedProfile.description} The desktop sends only this profile name.</div></div>
      <div className="settings-card full-card credential-card"><div className="card-heading"><span className="card-icon">◇</span><div><h3>Local tool access</h3><p>Add credentials only for tools you want local, approved terminal steps to use. Secrets stay encrypted on this Windows account.</p></div><span className="card-badge badge-private">Local only</span></div><div className="credential-status-grid"><div className={`credential-status ${hasCredential("tavily", "TAVILY_API_KEY") ? "status-configured" : ""}`}><span className="credential-status-icon">⌕</span><div><strong>Tavily Search</strong><small>{hasCredential("tavily", "TAVILY_API_KEY") ? "Configured on this PC" : "Not configured"}</small></div></div><div className={`credential-status ${hasCredential("github", "GITHUB_TOKEN") ? "status-configured" : ""}`}><span className="credential-status-icon">◈</span><div><strong>GitHub</strong><small>{hasCredential("github", "GITHUB_TOKEN") ? "Configured on this PC" : "Not configured"}</small></div></div><div className="credential-status"><span className="credential-status-icon">＋</span><div><strong>Custom tools</strong><small>{credentials.filter((credential) => credential.provider === "custom").length ? `${credentials.filter((credential) => credential.provider === "custom").length} saved locally` : "Optional"}</small></div></div></div><div className="credential-selected"><span className="eyebrow">ADD A LOCAL CREDENTIAL</span><span>{selectedCredential.description}</span></div><div className="credential-entry"><label>Tool<select value={credentialProvider} onChange={(event) => chooseCredentialProvider(event.target.value)}>{credentialPresets.map((preset) => <option value={preset.value} key={preset.value}>{preset.label}</option>)}</select></label><label>Environment name<input value={credentialName} onChange={(event) => setCredentialName(event.target.value.toUpperCase())} placeholder="TOOL_API_KEY" spellCheck={false} /></label><label>Secret value<input type="password" value={credentialSecret} onChange={(event) => setCredentialSecret(event.target.value)} placeholder="Paste locally — never uploaded" autoComplete="off" /></label><Button kind="primary" onClick={() => void saveCredential()} disabled={!isNativeDesktop || credentialBusy || !credentialName.trim() || !credentialSecret}>{credentialBusy ? "Saving…" : "Save locally"}</Button></div><div className="credential-list">{credentials.length === 0 ? <span className="credential-empty">No local credentials saved. Hosted provider keys are managed separately.</span> : credentials.map((credential) => <div className="credential-row" key={credential.name}><span className="credential-provider">{credential.provider}</span><strong>{credential.name}</strong><span>••••••••</span><Button kind="quiet" onClick={() => void removeCredential(credential.name)} disabled={credentialBusy}>Remove</Button></div>)}</div><div className="permission-note"><span>▣</span><span>Smara injects a selected alias only into the approved process and redacts it from output. It never sends this value to the hosted service.</span></div></div>
