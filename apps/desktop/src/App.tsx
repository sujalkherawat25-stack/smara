...
function App() {
  ...
  const loadTaskDetails = useCallback((taskId: string) => desktop.taskDetails(taskId), []);
  ...
  return <div className="app-shell">
    ...
    <main className="main-panel">
      <header className="topbar"><div className="topbar-title"><span className="eyebrow">LOCAL COMPANION</span><h1>{screen === "chat" ? "Chat" : screen === "activity" ? "Activity" : "Settings"}</h1></div><div className="topbar-actions">{isNativeDesktop && connection.paired && !connection.running && !connection.paused && <Button kind="primary" onClick={() => void runAction(desktop.start, "Desktop executor started. Approved local work can now be claimed.")}>Start executor</Button>}<StatusPill connection={connection} />{isNativeDesktop && !connection.has_cli_token && <Button kind="quiet" onClick={() => void signIn()} disabled={signingIn}>{signingIn ? "Signing in…" : "Sign in"}</Button>}<span className={`remote-pill ${remote?.ok ? "remote-online" : ""}`}><span className="status-dot" />{!isNativeDesktop ? "UI preview" : remote?.ok ? connection.has_cli_token ? "Hosted connected" : "Hosted ready · sign in" : loading ? "Checking…" : "Hosted offline"}</span><button className="icon-button" onClick={() => void refreshConnection()} aria-label="Refresh connection">↻</button></div></header>
      {notice && <div className="notice" role="status"><span>i</span>{notice}<button onClick={() => setNotice(null)} aria-label="Dismiss">×</button></div>}
      {screen === "chat" && <ChatScreen messages={messages} draft={draft} setDraft={setDraft} onSend={() => void send()} onStarter={(value) => void send(value)} streaming={streaming} activity={activity} canChat={connection.has_cli_token || connection.model_profile.startsWith("local:")} localModel={connection.model_profile.startsWith("local:")} connection={connection} onStartExecutor={() => void runAction(desktop.start, "Desktop executor started. Approved local work can now be claimed.")} onSignIn={() => setScreen("settings")} onOpenWeb={() => void desktop.openWeb()} onClearActivity={() => setActivity([])} transcriptEndRef={transcriptEndRef} />}
...
function ChatScreen({ messages, draft, setDraft, onSend, onStarter, streaming, activity, canChat, localModel, connection, onStartExecutor, onSignIn, onOpenWeb, onClearActivity, transcriptEndRef }: { messages: ChatMessage[]; draft: string; setDraft: (value: string) => void; onSend: () => void; onStarter: (value: string) => void; streaming: boolean; activity: ActivityItem[]; canChat: boolean; localModel: boolean; connection: ConnectionState; onStartExecutor: () => void; onSignIn: () => void; onOpenWeb: () => void; onClearActivity: () => void; transcriptEndRef: RefObject<HTMLDivElement> }) {
  ...
  const executorStopped = isNativeDesktop && connection.paired && !connection.running && !connection.paused;
...
function ActivityScreen({ connection, tasks, onLoadTaskDetails, onRefresh, onStart, onStop, onPause, onResume, onRevoke, onOpenWeb, onReadLog }: { connection: ConnectionState; tasks: TaskSummary[]; onLoadTaskDetails: (taskId: string) => Promise<TaskDetail>; onRefresh: () => void; onStart: () => void; onStop: () => void; onPause: () => void; onResume: () => void; onRevoke: () => void; onOpenWeb: () => void; onReadLog: () => Promise<string> }) {
