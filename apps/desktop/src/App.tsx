...
function ActivityScreen({ connection, tasks, onLoadTaskDetails, onRefresh, onStart, onStop, onPause, onResume, onRevoke, onOpenWeb, onReadLog, onDecideLocalTask }: { connection: ConnectionState; tasks: TaskSummary[]; onLoadTaskDetails: (taskId: string) => Promise<TaskDetail>; onRefresh: () => void; onStart: () => void; onStop: () => void; onPause: () => void; onResume: () => void; onRevoke: () => void; onOpenWeb: () => void; onReadLog: () => Promise<string>; onDecideLocalTask?: (taskId: string, approved: boolean) => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    previous?.remove();
    if (!selectedTask || selectedTask.status !== "waiting_approval" || selectedTask.approval_mode !== "desktop") return;
    ...
    message.innerHTML = "<strong>Approve on this Desktop</strong><p>This decision is local. Smara Web cannot release this task.</p>";
    const approve = document.createElement("button"); approve.className = "button button-primary"; approve.textContent = deciding ? "Working…" : "Approve locally"; approve.disabled = deciding;
    ...
    panel.append(message, approve, deny);
