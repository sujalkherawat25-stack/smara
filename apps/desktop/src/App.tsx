...
import type { ActivityItem, ChatEvent, ChatMessage, ConnectionState, LocalCredentialSummary, LocalModelProfile, Screen, TaskDetail, TaskSummary } from "./types";
...
function ArtifactResult({ artifact }: { artifact: TaskDetail["artifacts"][number] }) {
  ...
  return <details className="artifact-result" open={action === "local_file_write" || action === "local_file_preview"}>
    ...
    <div className="artifact-body">
      ...
      {!changedFiles.length && !artifacts.length && !documentResult && !diff && !result.output && <span className="detail-muted">Structured local result recorded. Open the full task history for the complete artifact.</span>}
    </div>
  </details>;
}
